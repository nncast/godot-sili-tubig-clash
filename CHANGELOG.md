...
# Changelog — round never ended when the Tubig side emptied out; Play Again

Verified against Godot 4.7.2 headless, not by inspection.

## "No Tubig left" never ended the round

Two cases were reported together. They are not the same bug, and only one of
them was broken:

- **All Tubig tagged** — already worked. Reproduced on this build, ends
  immediately, no change made.
- **All Tubig gone (quit)** — reproduced failing. Fixed.
...
does not remove a node until the END of the frame it was called in. So even the
refresh that `_on_player_left` defers still found the departing body sitting in
the group, alive and NORMAL.

The check therefore ran exactly one departure behind. Instrumented output from
the failing run:

```
[probe] _on_player_left 2
[probe] _check_for_sili_win entered, is_server=true tubigs=2
[probe] _on_player_left 3
[probe] _check_for_sili_win entered, is_server=true tubigs=1
```

The first check saw both players, the second saw the one that had just left, and
there was never a third — with no bodies left there are no `burned`/`died`
signals left to fire it. The round played its full clock out with nobody in it.

`_check_for_sili_win` now reads the live group through a new
`_live_tubig_bodies()` and skips anything already `is_queued_for_deletion()`: a
body marked for removal is not somebody who can still perform a rescue, whether
or not the engine has got round to freeing it. An empty list ends the round
through the same path as "everyone is burning" — deliberately not a special
case, because nobody left to catch is the same verdict as nobody left standing.

Also added a `MatchManager.is_running` guard at the top, so an empty pregame
roster cannot look like a Sili win on the way past. `end_match()` refused it
anyway; this just stops it being asked.

The `[match-end-debug]` prints added while hunting this are removed.

Covered by a new `tools/test_endgame_roster.tscn`.

## Play Again in the in-match settings panel

Added directly above the existing **Leave Game** button, which was left alone.

Host (or offline) gets the button. A client does not see it at all — it is
hidden, not disabled, and gets *"Waiting for host..."* instead. A greyed-out
button still reads as "something I could do if I got it right", and there is
nothing for a client to get right: the decision is not theirs. If the lobby
drops below `MIN_PRACTICE_PLAYERS`, the host's button greys out with *"Not
enough players to restart."* rather than silently doing nothing, because
`restart_round()` refuses at that size. State is recomputed on
`player_list_changed`, since somebody quitting can turn a restartable round into
an unstartable one while the panel is open.

It calls a new `NetworkManager.restart_round()`, deliberately **not**
`start_next_round()`. That advances the series rotation and hands the Sili to the
next player — correct at the end of a round that was played out, wrong for one
abandoned half-way: it would burn a turn nobody got to play and leave the
standings a result short with nothing to explain why. Re-running the same round
with the same Sili leaves `SeriesManager` untouched.

Covered by `tools/test_settings_actions.tscn`, including a real client branch
(an actual `ENetMultiplayerPeer` client, not an offline stand-in — offline is
treated AS host on purpose, so a null peer would exercise the wrong path and
pass for the wrong reason).

## test_endgame.tscn was already failing

Not caused by these changes — it spawned zero Tubigs on the untouched baseline
and failed on its first assertion. The arena-load handshake landed after that
file was written: `arena.gd` holds the round until every id in
`NetworkManager.players` reports in, and peers 2 and 3 in that test are
dictionary entries rather than real processes, so the round sat on the 20s
`READY_TIMEOUT` while the test asserted at frame 6. It now checks the fake peers
in by hand. All 11 assertions pass.

## Pre-existing failures, untouched

Verified by stashing these changes and re-running — identical counts either side:

| test | failures |
|---|---|
| `test_canopy_local_player` | 1 |
| `test_leaderboard_and_audio` | 1 |
| `test_replay_loading` | 2 |
| `test_title_screen` | 7 |
| `test_heat` | 6 |

`test_feed_and_disconnect` does not parse at all. It references
`MatchManager.FEED_SILI_COLOR`, `sili_name()` and `tubig_name()`, none of which
exist in the codebase any more — a test written against a removed API rather
than a broken test. Repairing it means rewriting it against whatever replaced
feed name-colouring, which is a separate job.
